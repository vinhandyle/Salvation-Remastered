...
public class TerminalMenuButton : MonoBehaviour
{
    ...
    public void LoadLevel(string extraLevel)
    {        
        if (extraLevel.Length > 0)
        {
            BossGauntlet.Instance.InitGauntlet(key, extraLevel);
            SceneController.Instance.LoadScene(extraLevel);
            SceneController.Instance.LoadScene(key, false);
        }
        else
        {
